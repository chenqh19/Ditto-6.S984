...
class service_10Handler : public service_10If {
private:
  uint64_t *_mem_data;
...
public:
  explicit service_10Handler(uint64_t *pointer_chasing_mem_data) {
  ...
  void rpc_10(const std::map<std::string, std::string> &carrier) override {
    TextMapReader reader(carrier);
    auto parent_span = opentracing::Tracer::Global()->Extract(reader);
    auto span = opentracing::Tracer::Global()->StartSpan(
        "rpc_10_server", {opentracing::ChildOf(parent_span->get())});
...
class service_10CloneFactory : public service_10IfFactory {
private:
  uint64_t *_pointer_chasing_mem_data;

public:
  explicit service_10CloneFactory(uint64_t *pointer_chasing_mem_data) {
    _pointer_chasing_mem_data = pointer_chasing_mem_data;
  }

  ~service_10CloneFactory() {}

  service_10If *
  getHandler(const ::apache::thrift::TConnectionInfo &connInfo) override {
    return new service_10Handler(_pointer_chasing_mem_data);
  }

  void releaseHandler(service_10If *handler) override { delete handler; }
};

void startServer(TServer &server) {
  cout << "Starting the service_10 server..." << endl;
  server.serve();
  cout << "Done." << endl;
}
...
int main(int argc, char *argv[]) {
  ...
  SetUpTracer("config/jaeger-config.yml", "service_10");
  int port = services_json["service_10"]["server_port"];
  TThreadedServer server(stdcxx::make_shared<service_10ProcessorFactory>(
                             stdcxx::make_shared<service_10CloneFactory>(
