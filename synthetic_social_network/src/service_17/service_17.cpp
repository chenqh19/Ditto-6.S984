...
public:
  ...
  void rpc_17(const std::map<std::string, std::string> &carrier) override {
    TextMapReader reader(carrier);
    auto parent_span = opentracing::Tracer::Global()->Extract(reader);
    auto span = opentracing::Tracer::Global()->StartSpan(
        "rpc_17_server", {opentracing::ChildOf(parent_span->get())});
    ...
    std::map<std::string, std::string> writer_text_map_18;
    TextMapWriter writer_18(writer_text_map_18);

    auto self_span_18 = opentracing::Tracer::Global()->StartSpan(
        "rpc_18_client", {opentracing::ChildOf(&(span->context()))});
    opentracing::Tracer::Global()->Inject(self_span_18->context(), writer_18);
    auto service_18_client_wrapper_18 = _service_18_client_pool->Pop();
    if (!service_18_client_wrapper_18) {
      LOG(error) << "ERROR: Failed to connect to service_18";
      ServiceException se;
      se.errorCode = ErrorCode::SE_THRIFT_CONN_ERROR;
      se.message = "Failed to connect to service_18";
      throw se;
    } else {
      try {
        service_18_client_wrapper_18->Send("Helloworld\n");
        _service_18_client_pool->Push(service_18_client_wrapper_18);
      } catch (...) {
        LOG(error) << "ERROR: Failed to send rpc.";
        _service_18_client_pool->Remove(service_18_client_wrapper_18);
      }
    }
    self_span_18->Finish();
...
public:
  ...
  void releaseHandler(service_17If *handler) override { delete handler; }
};

void startServer(TServer &server) {
  cout << "Starting the service_17 server..." << endl;
  server.serve();
  cout << "Done." << endl;
}
...
int main(int argc, char *argv[]) {
  ...
  SetUpTracer("config/jaeger-config.yml", "service_17");
  std::string service_18_addr = services_json["service_18"]["server_addr"];
  int service_18_port = services_json["service_18"]["server_port"];
  TCPClientPool<RedisTCPClient> service_18_client_pool(
