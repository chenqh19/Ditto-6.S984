...
class service_4Handler : public service_4If {
private:
  TCPClientPool<MemcachedTCPClient> *_service_5_client_pool;
  TCPClientPool<MongoTCPClient> *_service_6_client_pool;
  uint64_t *_mem_data;
...
public:
  explicit service_4Handler(
      TCPClientPool<MemcachedTCPClient> *service_5_client_pool,
      TCPClientPool<MongoTCPClient> *service_6_client_pool,
      uint64_t *pointer_chasing_mem_data) {
    _service_5_client_pool = service_5_client_pool;
    _service_6_client_pool = service_6_client_pool;
  ...
  void rpc_4(const std::map<std::string, std::string> &carrier) override {
    ...
    auto self_span_5 = opentracing::Tracer::Global()->StartSpan(
        "rpc_5_client", {opentracing::ChildOf(&(span->context()))});
    opentracing::Tracer::Global()->Inject(self_span_5->context(), writer_5);
    auto service_5_client_wrapper_5 = _service_5_client_pool->Pop();
    if (!service_5_client_wrapper_5) {
      LOG(error) << "ERROR: Failed to connect to service_5";
      ServiceException se;
      se.errorCode = ErrorCode::SE_THRIFT_CONN_ERROR;
      se.message = "Failed to connect to service_5";
      throw se;
    } else {
      try {
        service_5_client_wrapper_5->Send("Helloworld\n");
        _service_5_client_pool->Push(service_5_client_wrapper_5);
      } catch (...) {
        LOG(error) << "ERROR: Failed to send rpc.";
        _service_5_client_pool->Remove(service_5_client_wrapper_5);
      }
    }
    self_span_5->Finish();
...
class service_4CloneFactory : public service_4IfFactory {
private:
  TCPClientPool<MemcachedTCPClient> *_service_5_client_pool;
  TCPClientPool<MongoTCPClient> *_service_6_client_pool;
  uint64_t *_pointer_chasing_mem_data;
...
public:
  explicit service_4CloneFactory(
      TCPClientPool<MemcachedTCPClient> *service_5_client_pool,
      TCPClientPool<MongoTCPClient> *service_6_client_pool,
      uint64_t *pointer_chasing_mem_data) {
    _service_5_client_pool = service_5_client_pool;
  ...
  ~service_4CloneFactory() {}

  service_4If *
  getHandler(const ::apache::thrift::TConnectionInfo &connInfo) override {
    return new service_4Handler(_service_5_client_pool, _service_6_client_pool,
                                _pointer_chasing_mem_data);
  }

  void releaseHandler(service_4If *handler) override { delete handler; }
};

void startServer(TServer &server) {
  cout << "Starting the service_4 server..." << endl;
  server.serve();
  cout << "Done." << endl;
}
...
int main(int argc, char *argv[]) {
  ...
  TCPClientPool<MemcachedTCPClient> service_5_client_pool(
      "service_5", service_5_addr, service_5_port, 0, 512, 10000);

  std::string service_6_addr = services_json["service_6"]["server_addr"];
  int service_6_port = services_json["service_6"]["server_port"];
  TCPClientPool<MongoTCPClient> service_6_client_pool(
      "service_6", service_6_addr, service_6_port, 0, 512, 10000);

  int port = services_json["service_4"]["server_port"];
  TThreadedServer server(stdcxx::make_shared<service_4ProcessorFactory>(
                             stdcxx::make_shared<service_4CloneFactory>(
                                 &service_5_client_pool, &service_6_client_pool,
