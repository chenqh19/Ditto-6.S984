...
class service_9Handler : public service_9If {
private:
  uint64_t *_mem_data;
...
public:
  explicit service_9Handler(uint64_t *pointer_chasing_mem_data) {
  ...
  void rpc_9(const std::map<std::string, std::string> &carrier) override {
    TextMapReader reader(carrier);
    auto parent_span = opentracing::Tracer::Global()->Extract(reader);
    auto span = opentracing::Tracer::Global()->StartSpan(
        "rpc_9_server", {opentracing::ChildOf(parent_span->get())});
...
class service_9CloneFactory : public service_9IfFactory {
private:
  uint64_t *_pointer_chasing_mem_data;

public:
  explicit service_9CloneFactory(uint64_t *pointer_chasing_mem_data) {
    _pointer_chasing_mem_data = pointer_chasing_mem_data;
  }

  ~service_9CloneFactory() {}

  service_9If *
  getHandler(const ::apache::thrift::TConnectionInfo &connInfo) override {
    return new service_9Handler(_pointer_chasing_mem_data);
  }

  void releaseHandler(service_9If *handler) override { delete handler; }
};

void startServer(TServer &server) {
  cout << "Starting the service_9 server..." << endl;
  server.serve();
  cout << "Done." << endl;
}
...
int main(int argc, char *argv[]) {
  ...
  SetUpTracer("config/jaeger-config.yml", "service_9");
  int port = services_json["service_9"]["server_port"];
  TThreadedServer server(
      stdcxx::make_shared<service_9ProcessorFactory>(
          stdcxx::make_shared<service_9CloneFactory>(pointer_chasing_mem_data)),
