...
void TCPConn::Init(int epoll_fd, int socket_fd, uint64_t* mem_data, bool running_assembly)
{
    epoll_fd_ = epoll_fd;
...
void TCPConn::Process(uint64_t* curr_mem_addrs, uint64_t* curr_pointer_chasing_mem_addrs, uint64_t &request_id)
{
    ...
    {
        runAssembly0(mem_data_, request_id, curr_mem_addrs, curr_pointer_chasing_mem_addrs);
        request_id++;
