...
{
public:
    explicit WorkerThreadPool(int thread_number, uint64_t* mem_data);
    ~WorkerThreadPool();
...
template <typename T>
WorkerThreadPool<T>::WorkerThreadPool(int thread_number, uint64_t* mem_data) : mem_data_(mem_data), threads_(NULL)
{
    if (thread_number <= 0)
        throw std::exception();
...
public:
    ...
    NetworkThread(T* conns, int thread_index, int* epoll_fds, bool dispatch, WorkerThreadPool<T>* worker_thread_pool, FutureThreadPool* future_thread_pool, uint64_t* mem_data)
    {
        conns_ = conns;
...
public:
    ...
    NetworkThreadPool(T* conns, int* epoll_fds, int thread_number, bool dispatch, WorkerThreadPool<T>* worker_thread_pool, FutureThreadPool* future_thread_pool, uint64_t* mem_data)
    {
        if (thread_number <= 0)
            throw std::exception();
        threads_ = new pthread_t[thread_number];
        ...
        for (int i = 0; i < thread_number; ++i)
        {
            network_threads_.push_back(NetworkThread<T>(conns, i, epoll_fds, dispatch, worker_thread_pool, future_thread_pool, mem_data));
        }
