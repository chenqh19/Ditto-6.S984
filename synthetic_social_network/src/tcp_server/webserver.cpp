...
void WebServer::Init(int port, int network_thread_number, int worker_thread_number, bool dispatch, bool running_assembly)
{
    ...
    pointer_chasing_mem_data_ = new uint64_t[256 * 1024 * 1024 / 8];
    ...
    for (uint64_t i = 0; i < 23; i++) {
        ...
        for (uint64_t j = begin; j < end; j++) {
            pointer_chasing_mem_data_[j] = (uint64_t) &pointer_chasing_mem_data_[x[j - begin]];
        }
        begin = end;
    }
}
...
{
    // worker_thread_pool_ = new WorkerThreadPool<TCPConn>(worker_thread_number_, pointer_chasing_mem_data_);
    network_thread_pool_ = new NetworkThreadPool<TCPConn>(conns_, network_epoll_fds_, network_thread_number_, dispatch_, pointer_chasing_mem_data_);
    sleep(1);
}
...
bool WebServer::HandleNewConn(int epoll_fd)
{
    ...
    conns_[connfd].Init(epoll_fd, connfd, running_assembly_);

#ifdef MONGODB
    thread spawned_thread = thread(&blockingThread, connfd, &conns_[connfd], pointer_chasing_mem_data_);
